...
z_lens = np.array([0.295, 0.630, 0.722, 0.939, 1.004, 1.524])
h0_measured = np.array([72.5, 73.7, 74.1, 71.8, 73.3, 74.6])
h0_errors = np.array([2.1, 3.0, 2.6, 2.7, 2.1, 3.2])

# --- 3. Statistical Analysis --- CORRECTED WEIGHTED MEAN
print("\n[STEP 1] Performing full statistical analysis")

# Calculate weighted mean (CORRECTED)
weights = 1 / (h0_errors ** 2)
weighted_mean = np.sum(h0_measured * weights) / np.sum(weights)
...
tension = abs(weighted_mean - H0_opt) / weighted_std

# --- 4. Results ---
print("\n[STEP 2] Final results")
print(f"-> Model H0: {H0_opt:.2f} km/s/Mpc")
print(f"-> Weighted mean H0: {weighted_mean:.2f} ± {weighted_std:.2f} km/s/Mpc")
